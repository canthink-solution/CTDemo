...
use Sys\framework\Database\Interface\ConnectionInterface;
use Sys\framework\Database\Interface\BuilderCrudInterface;
use Sys\framework\Database\Interface\BuilderStatementInterface;

use Sys\framework\Database\Interface\QueryInterface;
use Sys\framework\Database\Interface\ResultInterface;
...
abstract class BaseDatabase extends DatabaseHelper implements ConnectionInterface, BuilderStatementInterface, QueryInterface, BuilderCrudInterface, ResultInterface
{
    ...
    public function where($columnName, $operator = NULL, $value = NULL)
    {
        try {
            ...
            $this->validateOperator(strtoupper($operator), ['LIKE', 'NOT LIKE']);
            ...
            if (is_array($columnName)) {
                foreach ($columnName as $column => $val) {
                    ...
                    $this->_buildWhereClause($column, $val, strtoupper($operator), 'AND');
                }
            } else {
                $this->_buildWhereClause($columnName, $value, strtoupper($operator), 'AND');
            }
        ...
        try {
            ...
            if (is_array($columnName)) {
                foreach ($columnName as $column => $val) {
                    ...
                    $this->_buildWhereClause($column, $val, strtoupper($operator), 'OR');
                }
            } else {
                $this->_buildWhereClause($columnName, $value, strtoupper($operator), 'OR');
            }
